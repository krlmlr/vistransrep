```{r 601-remove-all, include = FALSE}
rm(list = ls())
```
...
```
```{r 601-airlines-contain-detailed-information-for-each-airline}
airlines
```
...
```
```{r 601-flights-can-be-augmented-with-this-detailed-information}
flights %>%
  left_join(airlines) %>%
...
```
```{r 601-for-production-code-always-indicate-the-columns-to-join-by}
flights %>%
...
```
```{r 601-your-turn-join-the-planes-table}
planes
```
...
```
```{r 601-does-it-work-out-of-the-box}
try(
  ... %>%
    ..._join(...)
)
...
```
```{r 601-how-to-fix-and-also-turn-off-the-message}
try(
  ... %>%
    ..._join(..., ... = "...")
)
...
```
```{r 601-airports-contain-detailed-information-for-each-airport}
airports
...
```
```{r 601-in-this-case-explicit-by-columns-are-required}
flights %>%
...
```
```{r 601-left-vs-inner-vs-outer-joins-control-what-happens-in-case-of-mismatches}
flights %>%
...
```
```{r 601-your-turn-understand-mismatches-when-joining-the-planes-data}
try(
  ... %>%
    left_join(..., ... = "...") %>%
    filter(is.na(...))
...
```
```{r 601-verify-that-aa-has-more-mismatches-than-other-airlines}
try(
  ... %>%
    left_join(..., ... = "...") %>%
...
```
```{r 601-how-to-keep-only-the-flights-where-we-have-airplane-details}
try(
  ... %>%
    ..._join(..., ... = "...")
)
...
```
```{r 601-case-study-classification-of-mismatches}
flights %>%
...
```
```{r 601-before-joining-prepare-the-rhs-table-s}
origin_airports <-
  airports %>%
...
```
```{r 601-much-easier-with-the-prepared-tables}
flights %>%
...
```
```{r 601-your-turn-we-only-need-engines-and-seats-from-the-planes-table}
try({
  planes_join <-
    ... %>%
    ...(...)
...
```
```{r 601-either-lhs-or-rhs-should-have-a-key-as-part-of-by}
airports %>%
  count(faa)
...
```
```{r 601-your-turn-double-check-that-tailnum-is-indeed-a-key-in-planes}
try(
  ... %>%
    count(...) %>%
    ...(...)
...
```
```{r 601-create-artificial-dataset-where-airport-is-not-a-key}
dup_airports <-
...
```
```{r 601-your-turn-is-this-combination-a-key-why-why-not}
weather
...
```
```{r 601-how-to-find-the-offenders}
try(
  weather %>%
    count(...) %>%
    add_count(...) %>%
...
```
```{r 601-a-novel-approach-https-krlmlr-github-io-dm}
...
```
```{r 601-install-packages-devtools}
NA
```
