...
This is a dataset where principal statistical summaries 
...
Wide tables make better use of screen or paper space, and should be used when presenting 
...
```
```{r 401-deciding-what-goes-into-the-columns-what-into-the-rows}
table2 %>%
  xtabs(count ~ ., .) %>%
  ftable(col.vars = c("year", "type"))
```

The [{gt} package](https://gt.rstudio.com/) is a moden approach to present tables for formatting.
