...
```{r 18-data-can-have-color-in-bw-theme}
ggplot(
  data = mpg,
...
```{r 18-occasionally-useful-legend-at-the-bottom}
ggplot(
  data = mpg,
  mapping = aes(x = displ, y = hwy, color = class)
) +
...
```{r 18-theme-functions-overwrite-theme-if-called-afterwards}
ggplot(
  data = mpg,
  mapping = aes(x = displ, y = hwy, color = class)
) +
...
```{r 18-first-param-of-theme-functions-is-base-size}
ggplot(
  data = mpg,
  mapping = aes(x = displ, y = hwy, color = class)
) +
