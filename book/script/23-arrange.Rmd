...
```
```{r 23-arranging-two-means-first-the-first-then-within-duplicates-the-second}
flights %>%
...
```
```{r 23-filter-before-arranging-two}
flights %>%
  filter(dep_time < 600) %>%
  arrange(dep_time, dep_delay) %>%
  view()
...
```
```{r 23-two-filters-before-arranging-two}
flights %>%
  filter(dep_time < 600) %>%
  filter(month >= 10) %>%
  arrange(dep_time, dep_delay) %>%
...
```
```{r 23-arrange-with-regard-to-function-of-variable}
flights %>%
  filter(month == 4) %>%
...
```
```{r 23-two-filters-then-arrange-according-arr_time}
flights %>%
  filter(month == 4) %>%
  filter(day == 1) %>%
  arrange(arr_time) %>%
