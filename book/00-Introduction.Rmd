...
```{r 00-Introduction-1, echo = FALSE, fig.width=5}
...
```{r 00-Introduction-2, echo=FALSE}
knitr::include_graphics("img/toolkit.png")
```
...
```{R 00-Introduction-3, indeeddotcom, echo = F, fig.height = 6, fig.width = 9, dev = "svg"}
# The popularity data
pop_df <-
  data.frame(
...
```{r cars_basefig, dev="svg", fig.height=4.2}
...
```{r gapm_plot, dev="svg", fig.height=4.2}
library(ggplot2)
...
```{r 00-Introduction-5, echo=FALSE, fig.cap="RStudio dashboard metapher ^[Source: [Modern Dive](https://moderndive.com/)]" }
...
```{r 00-Introduction-6, echo=FALSE, fig.cap="R phone metapher ^[Source: [Modern Dive](https://moderndive.com/)]" }
knitr::include_graphics("img/r_vs_r_packages.png")
```
...
xdf = as.data.frame(x[ !is.na(x[ ,"Priority"]), c("Package", "Priority")][1:20, ]) %>% dplyr::filter(Priority == "base") %>%
  dplyr::mutate(Package = as.character(Package)) %>%
  print(n = 20)
```

```{r 00-Introduction-8, echo = FALSE}
xdf = as.data.frame(x[ !is.na(x[ ,"Priority"]), c("Package", "Priority")][1:20, ]) %>% dplyr::filter(Priority == "recommended") %>%
  print(n = 20)
```
...
```{r 00-Introduction-9, echo=FALSE}
knitr::include_graphics("img/rstudio_source_rec.png")
```
...
```{r 00-Introduction-10, echo=FALSE}
knitr::include_graphics("img/rstudio_source_arrow.png")
```
...
```{r 00-Introduction-11, echo=FALSE}
knitr::include_graphics("img/rstudio_source_ex.png")
```

RStudio will execute the command in the terminal.

```{r 00-Introduction-12, echo=FALSE}
knitr::include_graphics("img/rstudio_source_ex2.png")
```

You can see the new object in the _Environment_ pane.

```{r 00-Introduction-13, echo=FALSE}
knitr::include_graphics("img/rstudio_source_ex3.png")
```
...
```{r 00-Introduction-14, echo=FALSE}
knitr::include_graphics("img/rstudio_history.png")
```
...
```{r 00-Introduction-15, echo=FALSE}
...
```{r 00-Introduction-16, echo=FALSE}
knitr::include_graphics("img/rstudio_plots.png")
```
...
```{r 00-Introduction-17, echo=FALSE}
knitr::include_graphics("img/rstudio_packages.png")
```
...
```{r 00-Introduction-18, echo=FALSE}
knitr::include_graphics("img/rstudio_packages2.png")
```
...
```{r 00-Introduction-19, echo=FALSE}
knitr::include_graphics("img/rstudio_help.png")
```
...
```{r 00-Introduction-20, echo=FALSE}
