...
1. View the `trends` dataset in RStudio's data pane. Look up the meaning of the variables in the help.

    Hint: You need to load the `gfdata` package.

1. Find all observations from four years ago.

1. Find all observations that correspond to the `"Number of infant deaths: Median quantile"` indicator for the years 2000 to 2004, including.

1. Find all observations that correspond to any of the following indicators:
    - `"AIDS-related deaths, estimate"`
    - `"Malaria deaths, estimate"`
    - `"Maternal deaths, estimate"`

1. Are there any observations from the future?
...
Use a combined filter, an intermediate variable, a nested expression, and the pipe, for the following tasks. Which appeals more to you?

1. View all observations for the Central African Republic (`"CAF"`).

1. Restrict further to view all observations from the previous century.

1. Restrict further to view only the following indicators:
    - `"AIDS-related deaths, estimate"`
    - `"Maternal deaths, estimate"`

1. Restrict further to view only the observations where the indicator value was below 2500.

1. Restrict further to view only the observations where the indicator value has decreased compared to the previous year.
